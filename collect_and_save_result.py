...
def update_df_acc(file_name, folder_path, device, path):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--filename', nargs='+', type=str, default='arch_profile_mcts_cuda.csv', help='start system with test config')
    parser.add_argument('-d', '--device', type=str, default='gpu-v100', help='device used for profile')
    parser.add_argument('--folder_path', type=str, default='~/ray_results/train_mcts_hetero_cifar/train_mcts_hetero_cifar/', help='device used for profile')
    parser.add_argument('-p', '--imgpath', type=str, default='result_family_models', help='path to pdf image results')
    args = parser.parse_args()
    list_files = args.filename

    if type(list_files) is list:
        for csv_file in list_files:
            update_df_acc(csv_file, args.folder_path, args.device, args.imgpath) 
    else:
        update_df_acc(list_files, args.folder_path, args.device, args.imgpath) 