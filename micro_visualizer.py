...
def draw_graph(df):
    ...
    kmedoids = KMedoids(metric="euclidean", n_clusters=clusters, random_state=0).fit(
        np_data
    )
    ...
    for cluster_id in range(8):
        labels.append(
            Patch(
                facecolor=color_choice[cluster_id],
    ...
    f = ax.get_figure()
    f.savefig(
        "3D_lat_kmedioid.pdf", ext="pdf", bbox_extra_artists=(lgd,), bbox_inches="tight"
    )
    plt.show()
