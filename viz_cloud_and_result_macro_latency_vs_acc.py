...
def draw_acc_subplot(df, subplot, color, line=False):
    ...
    if line:
        ...
        line=""
    subplot.errorbar(x, y, xerr=e, fmt='.', mfc=color, mec=color,linestyle=line, ecolor=[color], label=args.device, color=color)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-fc', '--filename_cloud', type=str, default='op_gap_cloud/arch_profile_gpu-rtx2080_below_12_with_acc.csv', help='start system with test config')
    parser.add_argument('-fmcts', '--filename_mcts', type=str, default='mcts_generated/arch_profile_mcts_v6_t8_generated_cifar_macro_mcts_v6_sim_25_mcts_architecture_cpu_layers_with_acc.csv', help='start system with test config')
