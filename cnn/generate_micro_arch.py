...
def generate_micro(
    ...
    DARTS_network = Network(
        init_channels, CIFAR_CLASSES, layers, criterion, device=device
    )

    sampled_genotypes = DARTS_network.sample_genotypes(NUM_SAMPLE)
    df_with_lat = profile_sample_latencies(
        sampled_genotypes, init_channels, layers, auxiliary, drop_path_prob, device
    )
    # save after all data collected
    df_with_lat.to_csv(Path(filename + "_{}.csv".format(str(device))), index=None)
    ...
    df_centers.to_csv(Path(filename + "_{}_center.csv".format(str(device))), index=None)
    return df_centers


def profile_sample_latencies(
    sampled_genotypes, init_channels, layers, auxiliary, drop_path_prob, device
):
    ...
    for genotype in sampled_genotypes:
        ...
        model = NetworkCIFAR(init_channels, CIFAR_CLASSES, layers, auxiliary, genotype)
...
def kmedioid_grouping(df, nclusters):
    ...
    kmedoids = KMedoids(metric="euclidean", n_clusters=nclusters, random_state=0).fit(
        np_data
    )
